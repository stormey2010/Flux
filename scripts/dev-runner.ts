...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    // agent working inside Flux), these leak through and the child server
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  t3Home: Flag.string("home-dir").pipe(
    Flag.withDescription(
      "Explicit Flux data directory; runtime state is stored under userdata. Inside a git worktree this defaults to that worktree's own local state so dev state stays off the shared home.",
    ),
    Flag.optional,

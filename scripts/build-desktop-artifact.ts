...
const DESKTOP_APP_ID = "app.flux.desktop";
...
interface StagePackageJson {
  ...
  readonly fluxCommitHash: string;
...
export const DESKTOP_FILE_EXCLUSIONS = [
  // Flux always passes the user's installed Claude executable to the SDK,
...
export const stageResourceMonitor = Effect.fn("stageResourceMonitor")(function* (input: {
  ...
  const rustPathRemapPrefixes = [
    ...
    // Cargo parses RUSTFLAGS as a whitespace-delimited string on Windows, so
    // a prefix containing spaces would be split into invalid arguments. The
    // home prefix covers Cargo/Rustup and the normal checkout location while
    // remaining safe to pass through that parser.
    .filter((value) => !/\s/u.test(value));
  const rustFlags = [
    process.env.RUSTFLAGS?.trim(),
    ...rustPathRemapPrefixes.map((prefix) => `--remap-path-prefix=${prefix.trim()}=/flux-build`),
  ]
    .filter(Boolean)
    .join(" ");
  const rustBuildEnv = {
...
export function resolveDesktopProductName(version: string): string {
  return resolveDesktopUpdateChannel(version) === "nightly"
    ? "Flux (Nightly)"
    : (desktopPackageJson.productName ?? "Flux");
}
...
export const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  if (platform === "linux") {
    buildConfig.linux = {
      ...
      // flux:// OAuth callbacks to the app.
      protocols: [
        {
          name: "Flux",
          schemes: ["flux", "flux-dev"],
...
const stageWslNodePtyPrebuild = Effect.fn("stageWslNodePtyPrebuild")(function* (input: {
  ...
  yield* fs.writeFileString(path.join(prebuildDir, "flux-wsl-node-pty.json"), `${markerJson}\n`);
...
export const verifyWindowsPrimaryFffNativeLoad = Effect.fn(
  ...
  const probeRoot = yield* fs.makeTempDirectoryScoped({
    prefix: "flux-windows-primary-native-probe-",
  });
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const stageRoot = yield* mkdir({
    prefix: `flux-desktop-${options.platform}-stage-`,
  });
  ...
  if (!options.skipBuild) {
    ...
    // Keep the renderer's displayed version in lockstep with the packaged
    // desktop version. Vite reads APP_VERSION while the web bundle is built;
    // without passing it here, the web package fallback (currently 0.0.36)
    // gets embedded even when --build-version requests a newer release.
    const webBuildEnv = { ...process.env, APP_VERSION: appVersion };
  ...
  const stagePackageJson: StagePackageJson = {
    ...
    fluxCommitHash: commitHash,
    private: true,
    packageManager: rootPackageJson.packageManager,
    description: "Flux desktop build",
    author: "T3 Tools",
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for Flux."),

...
export const T3ProjectFileScript = Schema.Struct({
  name: trimmedNonEmpty({
    description: "Display name for the script, shown in the Flux scripts menu.",
  }),
  command: trimmedNonEmpty({
    description: "Shell command executed in a Flux terminal at the project root.",
  }),
  ...
  description: "A project script that team members can import into Flux.",
});
...
export const T3ProjectFile = Schema.Struct({
  ...
  iconPath: Schema.optionalKey(
    trimmedNonEmpty(
      {
        description:
          'Workspace-relative path to the project icon (e.g. "assets/logo.svg"). Checked before Flux\'s built-in icon locations.',
      },
  ...
  defaultThreadEnvMode: Schema.optionalKey(
    ThreadEnvMode.annotate({
      description:
        'Where new threads start for this repository: "worktree" for a fresh git worktree, "local" for the current checkout. A per-project setting in Flux overrides this; when neither is set, the global default applies.',
    }),
  ),
  scripts: Schema.optionalKey(
    Schema.Array(T3ProjectFileScript)
      .annotate({
        description: "Project scripts shared with everyone who opens this repository in Flux.",
      })
  ...
  description:
    "Checked-in project configuration for Flux (t3.json at the repository root). See https://t3.codes for documentation.",
});

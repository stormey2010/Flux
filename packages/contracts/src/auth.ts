...
export const AuthCreatePairingCredentialInput = Schema.Struct({
  // Short-lived pairing codes are intentionally constrained to one minute so
  // callers cannot accidentally create a long-lived credential for sharing.
  expiresInSeconds: Schema.optionalKey(Schema.Literal(60)),
  label: Schema.optionalKey(TrimmedNonEmptyString),

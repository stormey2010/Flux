 ...
 * relying on Flux's own orchestration projections, so usage stays complete
 * even for turns that were never driven through Flux. This mirrors the
 * approach `ccusage` takes. Cursor usage is pulled from Cursor's dashboard
 * usage-events API using the signed-in Cursor app token (no local transcript
 * directory).
 *
...
/**
 * Oldest usage contract that can still be merged without dropping the
 * provider buckets introduced by the current client.
 */
export const USAGE_MERGE_COMPATIBLE_SINCE = 4 as const;

export const UsageProviderKind = Schema.Literals(["claude", "codex", "cursor", "gemini"]);
...
/**
 * Compact plan/quota meters for the sidebar Usage popup.
 *
 * Separate from {@link UsageSummary}: that page is historical token/cost
 * activity; this snapshot is "how much of the plan is left right now".
 */
export const UsageQuotaProviderKind = Schema.Literals(["cursor", "codex", "gemini"]);
export type UsageQuotaProviderKind = typeof UsageQuotaProviderKind.Type;

export const UsageQuotaProviderStatus = Schema.Literals(["ok", "missing", "failed", "unavailable"]);
export type UsageQuotaProviderStatus = typeof UsageQuotaProviderStatus.Type;

export const UsageQuotaMeter = Schema.Struct({
  id: TrimmedNonEmptyString,
  label: TrimmedNonEmptyString,
  /** 0–100 when the provider reports a percentage window. */
  usedPercent: Schema.NullOr(Schema.Number),
  remainingPercent: Schema.NullOr(Schema.Number),
  /** Human short line such as `$14.60 of $20.00` or `12.5 credits`. */
  detail: Schema.NullOr(TrimmedNonEmptyString),
  /** ISO reset instant when known. */
  resetsAt: Schema.NullOr(Schema.String),
});
export type UsageQuotaMeter = typeof UsageQuotaMeter.Type;

export const UsageQuotaProvider = Schema.Struct({
  provider: UsageQuotaProviderKind,
  status: UsageQuotaProviderStatus,
  label: TrimmedNonEmptyString,
  planLabel: Schema.NullOr(TrimmedNonEmptyString),
  meters: Schema.Array(UsageQuotaMeter),
  message: Schema.NullOr(TrimmedNonEmptyString),
});
export type UsageQuotaProvider = typeof UsageQuotaProvider.Type;

export const UsageQuotaSummary = Schema.Struct({
  readAt: Schema.String,
  providers: Schema.Array(UsageQuotaProvider),
});
export type UsageQuotaSummary = typeof UsageQuotaSummary.Type;

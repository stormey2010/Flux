...
export const ClientSettingsSchema = Schema.Struct({
  ...
  // Composer chrome: remaining session/weekly usage for the thread's current
  // provider only. Off by default so the chat box stays quiet unless opted in.
  showProviderUsageInComposer: Schema.Boolean.pipe(
...
export const GeminiSettings = makeProviderSettingsSchema(
  {
    ...
    binaryPath: makeBinaryPathSetting("agy").pipe(
      Schema.annotateKey({
        title: "Binary path",
        description:
          "Path to Google's Antigravity CLI. Run `agy` once to sign in with the Google account tied to your AI Pro or Ultra subscription.",
...
export const OpenCodeSettings = makeProviderSettingsSchema(
  {
    ...
    serverUrl: TrimmedString.pipe(
      ...
      Schema.annotateKey({
        title: "Server URL",
        description: "Leave blank to let Flux spawn the server when needed.",
        providerSettingsForm: {
...
/**
 * A named delegation role made available to the coordinator. Profiles are
 * deliberately provider-neutral: the coordinator can use the native
 * subagent facility when the selected provider supports it, or run the role
 * as a separate T3-managed session when it does not.
 */
export const AgentProfile = Schema.Struct({
  ...
  instructions: TrimmedString.check(Schema.isMaxLength(12_000)),
  enabled: Schema.Boolean,
  primaryModel: Schema.NullOr(ModelSelection),
  backupModels: Schema.Array(ModelSelection).check(Schema.isMaxLength(5)),
  maxParallel: Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 8 })),
  timeoutSeconds: Schema.Int.check(Schema.isBetween({ minimum: 30, maximum: 3_600 })),
  allowNested: Schema.Boolean,
});
...
export const DEFAULT_AGENT_PROFILES: ReadonlyArray<AgentProfile> = [
  {
    id: "planner",
    name: "Planner",
    description: "Breaks a request into an implementation plan and dependencies.",
    instructions:
      "Analyze the request, inspect the repository, and return a concise, decision-complete plan. Do not modify files unless explicitly asked.",
    enabled: true,
    primaryModel: null,
    backupModels: [],
    maxParallel: 1,
  ...
  },
  {
    id: "frontend",
    name: "Frontend",
    description: "Owns UI, interaction, accessibility, and visual validation.",
    instructions:
      "Focus on frontend behavior and polish. Inspect existing components and patterns, implement the smallest coherent change, and verify responsive and accessible states.",
    enabled: true,
    primaryModel: null,
    backupModels: [],
    maxParallel: 2,
  ...
  },
  {
    id: "backend",
    name: "Backend",
    description: "Owns contracts, server behavior, persistence, and provider adapters.",
    instructions:
      "Focus on contracts, server behavior, persistence, and provider integration. Preserve validation and backwards compatibility, and add focused tests.",
    enabled: true,
    primaryModel: null,
    backupModels: [],
    maxParallel: 2,
  ...
  },
  {
    id: "testing",
    name: "Testing",
    description: "Finds regressions and writes focused tests for the requested behavior.",
    instructions:
      "Review the proposed change for edge cases and regressions. Add focused unit or integration coverage and report failures with actionable details.",
    enabled: true,
    primaryModel: null,
    backupModels: [],
    maxParallel: 2,
  ...
  },
  {
    id: "reviewer",
    name: "Reviewer",
    description: "Performs a final correctness, security, and maintainability review.",
    instructions:
      "Review the completed work like a maintainer. Prioritize correctness, security, user-visible regressions, and missing tests. Return findings ordered by severity.",
    enabled: true,
    primaryModel: null,
    backupModels: [],
    maxParallel: 1,
...
export const ServerSettings = Schema.Struct({
  ...
  /** Whether the coordinator should use the configured role catalog. */
  delegationEnabled: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  /** Additional coordinator guidance, kept separate from provider defaults. */
  delegationInstructions: TrimmedString.check(Schema.isMaxLength(12_000)).pipe(
    Schema.withDecodingDefault(
      Effect.succeed(
        "Act as a smart coordinator. Start with Planner for ambiguous or multi-part work, delegate independent frontend and backend tasks in parallel, run Testing after implementation, and ask Reviewer for a final pass. Give each role one focused outcome, avoid duplicate edits, and synthesize the results before replying.",
      ),
    ),
  ),
  /** Primary model used by the special "Default agent" chat entry. */
  coordinatorModel: Schema.NullOr(ModelSelection).pipe(
    Schema.withDecodingDefault(Effect.succeed(null)),
  ),
  /** Ordered coordinator fallbacks tried when its primary is unavailable. */
  coordinatorBackupModels: Schema.Array(ModelSelection)
    .check(Schema.isMaxLength(5))
    .pipe(Schema.withDecodingDefault(Effect.succeed([]))),
  /** Named roles and model fallback policy shown to the coordinator. */
  agentProfiles: Schema.Array(AgentProfile).pipe(
    Schema.withDecodingDefault(Effect.succeed(DEFAULT_AGENT_PROFILES)),
  ),
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  delegationInstructions: Schema.optionalKey(TrimmedString.check(Schema.isMaxLength(12_000))),
  coordinatorModel: Schema.optionalKey(Schema.NullOr(ModelSelection)),
  coordinatorBackupModels: Schema.optionalKey(
    Schema.Array(ModelSelection).check(Schema.isMaxLength(5)),
  ),
...
export const ClientSettingsPatch = Schema.Struct({
  ...
  showProviderUsageInComposer: Schema.optionalKey(Schema.Boolean),

...
# Invoked by ssh via SSH_ASKPASS when Flux re-runs ssh with a cached password
...
printf 'Flux ssh-askpass invoked without T3_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Flux re-runs\r
...
[Console]::Error.WriteLine("Flux ssh-askpass invoked without T3_SSH_AUTH_SECRET.")\r
exit 1\r
`;

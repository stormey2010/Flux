...
> {
  ...
  const resolvePathExecutable = Effect.gen(function* () {
    ...
    const pathDirectories = pathValue ? pathValue.split(delimiter) : [];
    // Windows' system installer normally places cloudflared here. Electron can
    // be launched with a reduced PATH, so keep this fallback local and only
    // use it after the user's normal PATH entries.
    const windowsInstallerDirectories =
      platform === "win32"
        ? ["C:\\Program Files\\cloudflared", "C:\\Program Files (x86)\\cloudflared"]
        : [];
    for (const directory of [...pathDirectories, ...windowsInstallerDirectories]) {
  ...
  const installUnlocked = Effect.fn("cloudflared.installUnlocked")(function* (
    ...
    if (!releaseAsset) {
      return yield* new RelayClientInstallError({
        ...
        message: `Flux does not provide a managed relay client binary for ${platform}-${arch}.`,
      });
    }

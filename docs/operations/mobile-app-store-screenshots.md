...
the software keyboard, and supplies deterministic Flux Connect discovery rows to the real
...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates Flux, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic Flux review diff.
...
The Pending rows use the production offline outbox and point at the real Flux and React fixture

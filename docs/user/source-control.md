...
Flux connects to your Git hosting provider so you can create pull requests, review code, and manage repositories without leaving the app.

## Supported Providers

Flux works with the platforms your team already uses:
...
- Flux can suggest titles and descriptions based on your commits
...
- Command-click (Control-click on Windows and Linux) a pull request number in the sidebar to open it in your browser instead of in Flux
...
1. Install the GitHub CLI (version 2.81.0 or newer) on the machine running Flux:
   ```bash
   brew install gh
   ```
2. Sign in:
   ...
   gh auth login
   ```
3. Open **Settings → Source Control** in Flux and verify GitHub shows as authenticated
...
If both are set, the access token wins. Restart Flux and verify the connection in **Source
Control settings**.
...
**Git is required** – Flux uses Git for all local operations. Ensure `git` is installed on your server.

**Server-side setup** – Authentication happens on the machine running Flux (the server), not your local browser. If you're using a hosted or team instance, your administrator may have already configured providers.

**Common issues:**

- **Provider shows "Not authenticated"** – Run the login command for that provider (e.g., `gh auth login`) in a terminal on the server, then rescan in Settings
- **GitHub says it could not verify sign-in status** – Flux needs GitHub CLI 2.81.0 or newer to check sign-in status. Update `gh` (e.g., `brew upgrade gh`), then rescan

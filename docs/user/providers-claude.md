...
This guide is for people who want to use more than one Claude setup in Flux. For Codex, see
[Codex](./providers-codex.md). For first-time setup, see [Install Flux](./install.md).
...
In Flux Settings, your Claude provider can stay like this:
...
An empty `CLAUDE_CONFIG_DIR path` means Flux uses Claude Code's normal config directory.

When you set this field, Flux points Claude Code at that directory with the
...
On web and desktop, when you return to an older Claude thread with a large context, Flux
...
Flux looks for Claude skills in the Claude config directory's `skills` folder, then
...
do not hand out raw thinking, so Flux asks for the summarized form and that summary is what you
...
`~/.claude_personal_home/.claude`, which is not where Flux looks.

Then add another Claude provider in Flux:
...
Flux only offers Claude providers that use the same config directory for an existing thread. A
...
multiple files under its config directory, so Flux keeps separate config directories isolated
...
Add or edit a Claude provider in Flux Settings:
...
Mark `ANTHROPIC_AUTH_TOKEN` as sensitive. Flux stores the value as a server secret and does not
...
Flux does not need a special Claude Code Router provider. Treat the router as a Claude

...
Use this when you want to connect to a Flux server from another device such as a phone, tablet, or separate desktop app.
...
run a headless server from the CLI, or have the desktop app launch Flux over SSH.
...
Use this when you want the desktop app to start or reuse Flux on another machine over SSH.
...
The remote host must have a compatible Node.js runtime. Flux uses the server package's `engines.node` requirement:

```text
^22.16 || ^23.11 || >=24.10
```

During SSH launch, Flux first checks whether `node` is on `PATH`. If it is missing, the launcher
...
If launch fails with `node: command not found`, a port-scan failure, or a message that the remote Node version does not satisfy the required range, SSH into the host and check the same non-interactive shell path Flux uses:
...
When the Flux web or desktop app and a remote server use different versions, a warning appears in
the conversation and in **Settings** → **Connections**. Follow the action shown there: Flux may
...
see [Keeping Flux in Sync](./updating.md).

On a Linux host, you can keep the server running after logout and manage it independently of the
connection method. See [Running Flux in the Background](./background-service.md).
...
Hosted pairing does not proxy traffic through Flux. The browser still connects directly to the backend URL in the pairing link.
...
### Deregister a Flux Connect Environment

Open your account menu and choose **Flux Connect** to see every environment registered to your
account. On mobile, open **Settings** → **Flux Connect**. Choose **Deregister** to revoke an
environment's Flux Connect access, remove any managed tunnel, and free its host space.

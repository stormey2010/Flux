# Keeping Flux in Sync

The Flux web or desktop app and the server it connects to work best when they use the same
version. If they do not match, Flux shows a warning with the right update option for that server.
...
| Action                     | What to do                                                                                                                                                               |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Update server**          | Available for the Flux Linux background service. Select the button and leave Flux open while it prepares, tests, restarts, and reconnects.                               |
| **Update the desktop app** | Open the Flux desktop app on the machine that runs the server and install the app update there. Reopen it if needed.                                                     |
| **Copy update command**    | Copy the command, open a terminal on the server machine, stop the current Flux server, and relaunch it with the copied command and any startup options you normally use. |

The available action depends on how that server was started. Flux does not update connected
...
If the server instead runs as the Flux background service, update the service on the host and
...
See [Running Flux in the Background](./background-service.md) for install, status, and removal
commands.

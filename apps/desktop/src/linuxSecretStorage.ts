...
function getGnomeKeyringRemediationMessage(): string {
  return "Flux could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart Flux.";
}

function getKWalletRemediationMessage(): string {
  return "Flux could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart Flux.";
}

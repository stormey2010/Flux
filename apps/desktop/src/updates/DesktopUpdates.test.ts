...
interface UpdatesHarnessOptions {
  ...
  readonly mainBranchCommit?: unknown;
}
...
function makeHarness(options: UpdatesHarnessOptions = {}) {
  ...
  const mainBranchResponse =
    options.mainBranchCommit === undefined
      ? new Response("{}", { status: 404 })
      : new Response(JSON.stringify(options.mainBranchCommit), {
          status: 200,
  ...
  const httpClientLayer = Layer.succeed(
    ...
    HttpClient.make((request) =>
      Effect.succeed(HttpClientResponse.fromWeb(request, mainBranchResponse.clone())),
    ),
  );
...
describe("DesktopUpdates", () => {
  ...
  it.effect("detects a newer commit on Flux main even without a published release", () => {
    const harness = makeHarness({
      env: { T3CODE_COMMIT_HASH: "1111111111111111111111111111111111111111" },
      mainBranchCommit: {
        sha: "2222222222222222222222222222222222222222",
        commit: {
          message: "feat: update the updater",

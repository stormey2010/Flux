      ...
      function* (failure: DesktopBackendManager.PreflightFailure) {
        ...
        if (!fatal) {
          ...
          yield* electronDialog.showErrorBox(
            ...
            `${reason}\n\nFlux will use the Windows backend for this launch and retry WSL the next time the app starts.`,
          );
        ...
        yield* electronDialog.showErrorBox(
          ...
          `${reason}\n\nFalling back to the Windows backend so Flux can open. Re-enable the WSL backend from Settings > Connections once the WSL distro is fixed.`,
        );

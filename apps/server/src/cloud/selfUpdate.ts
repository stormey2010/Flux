  ...
  const update: ServerSelfUpdate["Service"]["update"] = Effect.fn(
    ...
    if (capability === "desktop-managed") {
      return yield* failWith(
        "This server is managed by the Flux desktop app on its machine; update the desktop app to update it.",
      );
    }
    if (capability === null) {
      return yield* failWith(
        "Remote updates require the Flux background service. Run `t3 service install` on the server machine.",
      );
    }

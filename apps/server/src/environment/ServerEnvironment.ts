...
// The desktop artifact can build the server from a source checkout whose
// package.json version is intentionally different from the release version.
// Vite injects the artifact version at build time; direct/dev execution keeps
...
export const make = Effect.gen(function* () {
  ...
  const descriptor: ExecutionEnvironmentDescriptor = {
    ...
    serverVersion:
      typeof __T3CODE_BUILD_VERSION__ === "undefined"
        ? packageJson.version
        : __T3CODE_BUILD_VERSION__,
    capabilities: {

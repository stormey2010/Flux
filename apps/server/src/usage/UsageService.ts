 ...
 *
 * The scan reads the provider CLIs' own session files rather than Flux's
 * orchestration projections, so usage covers turns driven outside Flux too.
...
import { fetchCursorPeriodQuota, fetchCursorUsageRecords } from "./cursorUsage.ts";
import { fetchGeminiQuota } from "./geminiQuota.ts";
...
export class UsageService extends Context.Service<
  UsageService,
  {
    ...
    readonly readQuota: Effect.Effect<UsageQuotaSummary>;
  }
...
export const make = Effect.gen(function* () {
  ...
  const readSummary = Effect.fn("UsageService.readSummary")(function* (input: UsageSummaryInput) {
    ...
    // Cursor has no local JSONL transcripts. Pull dashboard usage-events the
    // same way community tools like `cursor-usage` do: local IDE session token
    // → GetFilteredUsageEvents. Soft-fail so Claude/Codex still render.
    {
      const untilDayExclusive = DateTime.make(`${input.untilDay}T00:00:00Z`);
      const cursorEndMs =
        hourlyWindow?.untilTimeMs ??
        (Option.isSome(untilDayExclusive)
          ? DateTime.toEpochMillis(untilDayExclusive.value) + 24 * 60 * 60 * 1000
          : startedAtMs + 24 * 60 * 60 * 1000);
      const cursorStartMs = hourlyWindow?.sinceTimeMs ?? DateTime.toEpochMillis(windowStart.value);
      ...
      sources.push({
        fingerprint: {
          hostId,
          provider: "cursor",
          resolvedHomePath: cursor.resolvedHomePath,
          volumeId: "",
        },
        status: cursor.status,
        scannedFiles: cursor.scannedFiles,
        skippedFiles: cursor.skippedFiles,
        malformedRecords: cursor.malformedRecords,
        distinctSessions: sessionIds.size,
  ...
  const readQuota = Effect.fn("UsageService.readQuota")(function* () {
    const [cursor, codex, gemini] = yield* Effect.all(
      [
        Effect.promise(() => fetchCursorPeriodQuota()),
        fetchCodexQuota.pipe(
    ...
    } satisfies UsageQuotaSummary;
  });

  // `readQuota` is a zero-argument Effect.fn used to keep the probe lazy, but
  // the service contract exposes the materialized Effect consumed directly by
  // the WebSocket RPC handler.
  return { readSummary, readQuota: readQuota() } as const;
});

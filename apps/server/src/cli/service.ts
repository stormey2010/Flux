...
const serviceInstallCommand = Command.make("install", projectLocationFlags).pipe(
  Command.withDescription("Install Flux as a background service for this user."),
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        ...
        if (!result.changed) {
          yield* Console.log(`Flux service is already installed with t3@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Flux service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
        );
      }),
    ),
  ),
  ...
  Command.withHandler((flags) =>
    runServiceCommand(
      ...
      Effect.gen(function* () {
        const result = yield* reconcileService();
        if (!result.changed) {
          yield* Console.log(`Flux service is already using t3@${packageJson.version}.`);
          return;
        }
        yield* Console.log(
          `${result.previouslyInstalled ? "Updated" : "Installed"} Flux service with t3@${packageJson.version}.\nLogs: ${result.plan.logPath}`,
...
const serviceUninstallCommand = Command.make("uninstall", projectLocationFlags).pipe(
  Command.withDescription("Stop and remove the Flux background service."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
        ...
        yield* Console.log(
          removed ? "Removed the Flux service." : "Flux service is not installed.",
        );
      }),
    ),
  ),
...
const serviceStatusCommand = Command.make("status", projectLocationFlags).pipe(
  Command.withDescription("Show whether the Flux background service is installed."),
  Command.withHandler((flags) =>
    runServiceCommand(
      flags,
      Effect.gen(function* () {
...
export const offerServiceDuringOnboarding = Effect.gen(function* () {
  ...
  if (installed && current) {
    yield* Console.log("Flux is already set up to run in the background on this machine.");
    return true;
  }
  ...
  const wanted = yield* Prompt.run(
    Prompt.confirm({
      message: installed
        ? "The installed Flux service needs an update or repair. Update it now?"
        : platform === "darwin"
          ? "Run Flux in the background whenever you log in to this Mac? " +
            "It stays reachable through Flux Connect while you are logged in."
          : "Run Flux in the background whenever this machine boots? " +
            "It stays reachable through Flux Connect even after you log out.",
      initial: true,
    }),
  );
  if (!wanted) {
...
export const serviceCommand = Command.make("service").pipe(
  Command.withDescription("Manage the Flux background service."),

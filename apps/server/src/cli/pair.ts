...
export class NoRunningServerError extends Schema.TaggedErrorClass<NoRunningServerError>()(
  ...
  override get message(): string {
    return [
      "No running Flux server found.",
      ...this.checkedStatePaths.map((statePath) => `  checked ${statePath}`),
      "Start one with `npx t3 serve`, or connect this machine with Flux Connect: `npx t3 connect`.",
    ].join("\n");
  }
}
...
export class ServesOtherEnvironmentError extends Schema.TaggedErrorClass<ServesOtherEnvironmentError>()(
  ...
  override get message(): string {
    return `Tailscale Serve on HTTPS port ${String(this.servePort)} already fronts a different Flux server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export class ServePortOccupiedError extends Schema.TaggedErrorClass<ServePortOccupiedError>()(
  ...
  override get message(): string {
    return `HTTPS port ${String(this.servePort)} on the tailnet already serves something that is not a Flux server. Pass --tailscale-serve-port to publish this one on another port.`;
  }
}
...
export const pairCommand = Command.make("pair", {
  ...
  Command.withDescription(
    "Mint a pairing token for a running Flux server and print it as a QR code.",
  ),

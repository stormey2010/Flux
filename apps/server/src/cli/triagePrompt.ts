...
export const TRIAGE_PLAYBOOK = `# Flux triage playbook

You are a support engineer for Flux (https://github.com/pingdotgg/t3code), working
...
- How is Flux running on this machine: \`npx t3 serve\` in a terminal, the
...
Do not patch the Flux source as a fix. A good issue with strong repro steps
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Flux triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `A Flux user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Flux triage context

...
const makeTest = (overrides: DeepPartial<ServerSettings> = {}) =>
  Effect.gen(function* () {
    const { automaticGitFetchInterval, providerHealthRefreshInterval, ...overridesForMerge } =
      overrides;
    // Keep provider unit tests focused on their own prompts unless delegation
    // is explicitly enabled by the test. Production settings retain the
    // default coordinator behavior from DEFAULT_SERVER_SETTINGS.

...
## Flux collaborative browser

You are running inside Flux. The \`t3-code\` MCP server is the product-native collaborative browser shared with the user. When it exposes \`preview_*\` tools, prefer those tools for browser navigation, inspection, interaction, screenshots, and recordings.
...
export function buildCodexDeveloperInstructions(
  ...
  const delegation = delegationInstructions.trim();
  return `${base}

<runtime_info>In case you're asked: you are running in Flux through the Codex harness, as ${toSingleLine(runtime.model)} with ${toSingleLine(runtime.reasoningEffort)} reasoning effort. No need to mention this otherwise.</runtime_info>${delegation ? `\n\n${delegation}` : ""}`;
}

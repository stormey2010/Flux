...
describe("Codex active-turn steering", () => {
  ...
  it("only resolves a steer target for a running session", () => {
    const activeTurnId = TurnId.make("provider-turn-active");
  ...
  it("does not revive a turn that settled while steer was in flight", () => {
    const activeTurnId = TurnId.make("provider-turn-active");
    NodeAssert.equal(
...
describe("buildCodexDeveloperInstructions", () => {
  it("appends runtime info after the mode instructions", () => {
    ...
    NodeAssert.match(instructions, /Flux/);
...
describe("T3 browser developer instructions", () => {
  ...
  it("omits the browser block entirely when the preview tools are not attached", () => {
    for (const instructions of [
      ...
      NodeAssert.doesNotMatch(instructions, /Flux collaborative browser/);

...
interface ClaudeSessionContext {
  ...
  /** Cached coordinator role catalog for this provider session. */
...
const buildUserMessageEffect = Effect.fn("buildUserMessageEffect")(function* (
  ...
  const delegation = dependencies.delegationInstructions?.trim();
  const text = delegation ? `${delegation}\n\n${promptText}` : promptText;
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const delegationInstructions = yield* serverSettings.getSettings.pipe(
        Effect.map(buildAgentDelegationInstructions),
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    const message = yield* buildUserMessageEffect(input, {
      ...
      boundInstanceId,
      delegationInstructions: context.delegationInstructions,
    });

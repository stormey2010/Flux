...
const CURSOR_ACP_MODEL_DISCOVERY_FAILED_MESSAGE = [
  ...
  "Cursor CLI setup may be incomplete; install or enable the Cursor CLI, restart Flux, and try again.",
    ...
    if (!cursorSettings.enabled) {
      return buildServerProvider({
        ...
        probe: {
          ...
          status: "warning",
          auth: { status: "unknown" },
          message: "Cursor is disabled in Flux settings.",
        },
      });
    }
...
function buildCursorCliCommandMissingMessage(binaryPath: string): string {
  return [
    ...
    `Install or enable the Cursor CLI, make sure \`${binaryPath}\` is on PATH, then restart Flux.`,
    `See ${CURSOR_CLI_INSTALLATION_DOCS_URL}.`,
  ].join(" ");
}

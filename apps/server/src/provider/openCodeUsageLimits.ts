...
/**
 * Connected providers this feature can report usage for. Only the
 * OpenCode-managed providers (Go / Zen) expose usage fields, so BYO upstreams
 * such as OpenAI or Anthropic are deliberately excluded — they have no usage to
 * be "missing", and callers must not warn about them.
 */
export function connectedManagedOpenCodeProviders(
  inventory: OpenCodeInventory,
): ReadonlyArray<ManagedProviderRecord> {
  const connected = new Set(inventory.providerList.connected);
  return inventory.providerList.all.filter(
    (provider) =>
      connected.has(provider.id) && getOpenCodeManagedProviderDescriptor(provider.id) !== undefined,
  );
}
...
export function resolveOpenCodeManagedUsageLimits(input: {
  ...
  const windows = connectedManagedOpenCodeProviders(input.inventory).flatMap((provider) => {

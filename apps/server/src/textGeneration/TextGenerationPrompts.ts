...
const INITIAL_THREAD_TITLE_PROMPT = `Generate a title that will help the user recognize this Flux thread weeks later.
...
function regenerateThreadTitlePrompt(previousTitle: string): string {
  return `Regenerate the title for an existing Flux thread so the user can recognize it weeks later.

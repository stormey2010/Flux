...
describe("buildThreadTitlePrompt", () => {
  it("includes the user message in the prompt", () => {
    ...
    expect(result.prompt).toContain(
      "Generate a title that will help the user recognize this Flux thread weeks later.",
    );
    expect(result.prompt).toContain(
  ...
  it("regenerates from recent thread contents and identifies the previous title", () => {
    ...
    expect(result.prompt).toContain(
      "Regenerate the title for an existing Flux thread so the user can recognize it weeks later.",
    );

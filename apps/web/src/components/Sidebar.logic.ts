...
export function shouldCreateNewThreadInCurrentProject(
  _shiftKey: boolean,
  _projectGroupCount: number,
): boolean {
  // Always open the project picker (or create-project flow) so new chats start
  // from an explicit project choice, never a silent checkout default.
  return false;
}

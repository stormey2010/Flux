...
export function ThemeLibrary({
  ...
  const notifyThemeSaveFailure = useCallback(() => {
    toastManager.add(
      stackedThreadToast({
        type: "error",
        title: "Couldnâ€™t save theme selection",
  ...
  // shows Flux selected instead of nothing.
  ...
  const renderPairGrid = () => (
    ...
    // accepted â€” scoping the group tighter makes the handoffs feel sluggish.
    <TooltipProvider>
      <div
  ...
  return (
    <div className="space-y-3">
      <p className="px-3 text-[13px] leading-[1.45] text-muted-foreground/80 sm:px-4">
        Choose how Flux looks. Use a built-in theme or make your own.
      </p>
          ...
          if (modes.length === 1) {
            ...
            toastManager.add(
              stackedThreadToast({
                ...
                description: `Itâ€™s now your ${modes[0]!} theme.`,
              }),
            );
            return true;
          }
      ...
      <AlertDialog open={isThemeRemovalOpen} onOpenChange={setIsThemeRemovalOpen}>
        <AlertDialogPopup>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {canRemoveCollection
                ? `Remove themes from â€œ${removeDialogCollectionLabel}â€?`
                : `Remove â€œ${removeDialogTheme?.label}â€?`}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {canRemoveCollection

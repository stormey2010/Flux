...
const ConnectedClientListRow = memo(function ConnectedClientListRow({
  ...
  const primaryLabel =
    ...
    ([clientSession.client.os, clientSession.client.browser].filter(Boolean).join(" Â· ") ||
...
function EnvironmentWebUrl({ url }: { readonly url: string | null }) {
  const { copyToClipboard, isCopied } = useCopyToClipboard<{ url: string }>({
    target: "web app URL",
    onCopy: () => {
      toastManager.add({ type: "success", title: "Web app URL copied" });
    },
  });
  if (!url) return null;
  const open = () => {
  ...
  return (
    <div className="flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground">
      ...
      <button
        type="button"
        className="min-w-0 truncate text-left underline decoration-border underline-offset-2 hover:text-foreground"
      ...
      <button
        ...
        aria-label="Open web app URL"
        title="Open web app URL"
        onClick={open}
      >
        {isCopied ? (
          <CopyIcon className="size-3 text-success" />
        ) : (
...
function CloudflareTunnelRow() {
  const desktopBridge = window.desktopBridge;
  const readState = desktopBridge?.getCloudflareTunnelState;
  const createManagedTunnel = desktopBridge?.createCloudflareTunnel;
  const connectCloudflare = desktopBridge?.connectCloudflare;
  const disconnectCloudflare = desktopBridge?.disconnectCloudflare;
  const deleteCloudflareTunnel = desktopBridge?.deleteCloudflareTunnel;
  const [tunnelState, setTunnelState] = useState<DesktopCloudflareTunnelState | null>(null);
  const [isUpdating, setIsUpdating] = useState(false);
  const [isCreateDialogOpen, setIsCreateDialogOpen] = useState(false);
  const [hostnameInput, setHostnameInput] = useState("flux");
  ...
  const refreshState = useCallback(async () => {
    if (!readState) return;
    try {
      setTunnelState(await readState());
    } catch {
      // Older desktop shells simply do not expose the optional tunnel bridge.
    }
  }, [readState]);
  ...
  useEffect(() => {
    if (!readState) return;
    void refreshState();
    const timer = window.setInterval(() => void refreshState(), 2_000);
    return () => window.clearInterval(timer);
  }, [readState, refreshState]);

  if (!createManagedTunnel || !connectCloudflare) return null;

  const isRunning = tunnelState?.status === "running";
  const isCreating = isUpdating || tunnelState?.status === "starting";
  const handleCreateManagedTunnel = async () => {
    setIsUpdating(true);
    try {
      const hostname = hostnameInput.trim();
      const next = await createManagedTunnel({
        ...(hostname ? { hostname } : {}),
        ...(cloudflareAccountId ? { accountId: cloudflareAccountId } : {}),
        ...(cloudflareZoneId ? { zoneId: cloudflareZoneId } : {}),
      });
      if (next.status === "error")
        throw new Error(next.error ?? "Cloudflare could not be configured.");
      setTunnelState(next);
      ...
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Could not create Flux Tunnel",
          description:
            error instanceof Error ? error.message : "Cloudflare could not be configured.",
        }),
      );
    } finally {
      setIsUpdating(false);
  ...
  const handleConnectCloudflare = async () => {
    setIsUpdating(true);
    try {
      const next = await connectCloudflare();
      if (next.status === "error") throw new Error(next.error ?? "Cloudflare sign-in failed.");
      setTunnelState(next);
      setCloudflareAccountId(next.cloudflareAccountId ?? next.cloudflareAccounts?.[0]?.id ?? "");
      setCloudflareZoneId(next.cloudflareZoneId ?? next.cloudflareZones?.[0]?.id ?? "");
      await refreshState();
    } catch (error) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Could not connect Cloudflare",
          description: error instanceof Error ? error.message : "Cloudflare sign-in failed.",
        }),
      );
    } finally {
      setIsUpdating(false);
  ...
  const handleDisconnectCloudflare = async () => {
    if (!disconnectCloudflare) return;
    setIsUpdating(true);
    try {
  ...
  const handleDeleteCloudflareTunnel = async () => {
    if (!deleteCloudflareTunnel) return;
    setIsUpdating(true);
    try {
  ...
  return (
    <>
      <SettingsRow
        ...
        description={
          isCreating
            ? `Creating ${tunnelState?.hostname ?? "your Flux Tunnel"}: Cloudflare is configuring the tunnel and verifying its DNS record…`
            : isRunning && tunnelState?.publicUrl
              ? "Flux is reachable through the active Cloudflare URL."
              : "Connect Cloudflare once, choose a hostname, and Flux will create the secure public URL."
        }
        status={
          tunnelState?.status === "error" ? (
            <span className="block text-destructive">{tunnelState.error ?? "Tunnel stopped."}</span>
          ) : null
        }
        control={
          <div className="flex w-full flex-col items-stretch gap-2 sm:w-auto sm:items-end">
            {tunnelState?.publicUrl ? <EnvironmentWebUrl url={tunnelState.publicUrl} /> : null}
            <div className="flex flex-wrap items-center justify-end gap-2">
              <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                <DialogTrigger
                  render={
                    <Button size="xs" variant="default" disabled={isCreating || isRunning}>
                ...
                <DialogPopup className="max-w-md">
                  <DialogHeader>
                    <DialogTitle>Create a Flux Tunnel</DialogTitle>
                    <DialogDescription>
                      Connect your Cloudflare account, choose a domain, and Flux will create the
                      managed tunnel, DNS route, and public URL automatically.
                    </DialogDescription>
                  </DialogHeader>
                  <DialogPanel className="space-y-4">
                    {tunnelState?.status === "error" && tunnelState.error ? (
                      <p className="rounded-lg border border-destructive/35 bg-destructive/8 px-3 py-2 text-xs text-destructive">
                        {tunnelState.error}
                      </p>
                    ) : null}
                    {!tunnelState?.cloudflareConnected ? (
                      <div className="rounded-lg border border-border/60 bg-muted/25 px-3 py-3 text-xs text-muted-foreground">
                        <p>
                          Step 1: connect Cloudflare in your browser. Flux only requests access
                          needed to create and manage this tunnel and its DNS record.
                        </p>
                      </div>
                    ) : null}
                    {tunnelState?.cloudflareConnected ? (
                      <>
                        <label className="block">
                          ...
                          <select
                            ...
                            value={cloudflareAccountId || tunnelState.cloudflareAccountId || ""}
                            onChange={(event) => setCloudflareAccountId(event.target.value)}
                          >
                            {(tunnelState.cloudflareAccounts ?? []).map((account) => (
                        ...
                        <label className="block">
                          ...
                          <select
                            className="h-8 w-full rounded-md border border-border bg-background px-2 text-sm"
                            value={cloudflareZoneId || tunnelState.cloudflareZoneId || ""}
                            onChange={(event) => setCloudflareZoneId(event.target.value)}
                          >
                            {(tunnelState.cloudflareZones ?? []).map((zone) => (
                    ...
                    <label className="block">
                      ...
                      <Input
                        ...
                        placeholder="flux or flux.example.com"
                        disabled={isUpdating}
                        autoFocus
                      />
                      <span className="mt-1.5 block text-xs text-muted-foreground">
                        Enter <code>flux</code> to use the domain you select in Cloudflare, or a
                        full hostname such as <code>flux.example.com</code>.
                      </span>
                    </label>
                    <p className="rounded-lg border border-border/60 bg-muted/25 px-3 py-2 text-xs text-muted-foreground">
                      Flux remembers the tunnel and DNS record it creates. Existing records are
                      never overwritten; choose another subdomain if one already exists.
                    </p>
                  </DialogPanel>
                  <DialogFooter variant="bare">
                    ...
                    {tunnelState?.cloudflareConnected ? (
                      <Button
                        disabled={isCreating}
                        onClick={() => void handleCreateManagedTunnel()}
                      >
                        {isCreating ? "Creating tunnel…" : "Create managed tunnel"}
                      </Button>
                    ) : (
                      <Button disabled={isUpdating} onClick={() => void handleConnectCloudflare()}>
                        {isUpdating ? "Opening Cloudflare…" : "Connect Cloudflare"}
                      </Button>
                    )}
                  </DialogFooter>
                </DialogPopup>
              </Dialog>
              {tunnelState?.cloudflareConnected && disconnectCloudflare ? (
                <Button
                  size="xs"
                  variant="ghost"
                  disabled={isUpdating}
              ...
              {tunnelState?.tunnelId && deleteCloudflareTunnel ? (
                <Button
                  size="xs"
                  variant="ghost"
                  disabled={isUpdating}
                  onClick={() => void handleDeleteCloudflareTunnel()}
                >
                  Delete Flux tunnel
                </Button>
              ) : null}
      ...
      />
      <FluxTunnelPairingRow tunnelUrl={tunnelState?.publicUrl ?? null} />
    </>
  );
}

function FluxTunnelPairingRow({ tunnelUrl }: { tunnelUrl: string | null }) {
  const [isOpen, setIsOpen] = useState(false);
  const [pairingCode, setPairingCode] = useState<string | null>(null);
  const [pairingUrl, setPairingUrl] = useState<string | null>(null);
  const [pairingLinkId, setPairingLinkId] = useState<string | null>(null);
  const [knownSessionIds, setKnownSessionIds] = useState<ReadonlySet<string> | null>(null);
  const [pairingStatus, setPairingStatus] = useState<"ready" | "used" | "connected">("ready");
  const [expiresAtMs, setExpiresAtMs] = useState<number | null>(null);
  const [isCreating, setIsCreating] = useState(false);
  const nowMs = useRelativeTimeTick(1_000);
  const { copyToClipboard, isCopied } = useCopyToClipboard<{ value: string }>({
    target: "Flux pairing code",
    onCopy: () => toastManager.add({ type: "success", title: "Pairing code copied" }),
  });

  const createPairingCode = async () => {
    try {
      setIsCreating(true);
      const [issued, sessions] = await Promise.all([
        createServerPairingCredential({
          label: "One-minute web app pairing",
          expiresInSeconds: 60,
        }),
        listServerClientSessions(),
      ]);
      setPairingCode(issued.credential);
      setPairingLinkId(issued.id);
      setKnownSessionIds(new Set(sessions.map((session) => session.sessionId)));
      setPairingStatus("ready");
      setPairingUrl(tunnelUrl ? resolveDesktopPairingUrl(tunnelUrl, issued.credential) : null);
      setExpiresAtMs(Date.now() + 60_000);
      toastManager.add({ type: "success", title: "Pairing code created" });
    } catch (error) {
      toastManager.add(
        stackedThreadToast({
          type: "error",
          title: "Could not create pairing code",
          description: error instanceof Error ? error.message : "Enter a valid Flux web app URL.",
        }),
      );
    } finally {
      setIsCreating(false);
  ...
  const secondsRemaining =
    expiresAtMs === null ? null : Math.max(0, Math.ceil((expiresAtMs - nowMs) / 1_000));
  const qrValue = pairingUrl ?? pairingCode;
  useEffect(() => {
    if (!pairingLinkId || !knownSessionIds || secondsRemaining === null || secondsRemaining <= 0)
      return;
    const refresh = async () => {
      try {
        ...
        if (sessions.some((session) => !knownSessionIds.has(session.sessionId))) {
          setPairingStatus("connected");
          return;
        }
        if (!links.some((link) => link.id === pairingLinkId)) setPairingStatus("used");
      } catch {
        // Keep the code usable if a background status refresh is temporarily unavailable.
      }
    };
    void refresh();
    const timer = window.setInterval(() => void refresh(), 1_000);
    return () => window.clearInterval(timer);
  }, [knownSessionIds, pairingLinkId, secondsRemaining]);
  const handleDialogOpenChange = (open: boolean) => {
    setIsOpen(open);
    if (!open) {
      setPairingCode(null);
      setPairingUrl(null);
      setPairingLinkId(null);
      setKnownSessionIds(null);
      setExpiresAtMs(null);
    }
  };
  ...
  return (
    <SettingsRow
      title="Pairing"
      description="Generate a one-minute pairing code, then copy it or scan it with Flux."
      control={
        <div className="flex flex-col items-stretch gap-2 sm:items-end">
          <Dialog open={isOpen} onOpenChange={handleDialogOpenChange}>
            ...
            <DialogPopup className="max-w-md">
              <DialogHeader>
                <DialogTitle>Pair another Flux app</DialogTitle>
                <DialogDescription>
                  Generate a one-time code. Scan the QR code in Flux, or paste the code manually.
                </DialogDescription>
              </DialogHeader>
                  ...
                  <div className="rounded-lg border border-primary/30 bg-primary/5 p-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {secondsRemaining === 0 ? "Expired" : `Expires in ${secondsRemaining}s`}
                      </span>
                    </div>
                    <div className="mt-2 flex items-center gap-2">
                      <code className="min-w-0 flex-1 truncate rounded bg-background px-2 py-1.5 font-mono text-sm text-foreground">
                        {pairingCode}
                      </code>
                      <Button
                        size="xs"
                        ...
                        onClick={() => copyToClipboard(pairingCode, { value: pairingCode })}
                      >
                    ...
                    {pairingUrl ? (
                      ...
                      <div className="mt-3 flex justify-center rounded-xl bg-white p-3">
                        <QRCodeSvg
                          ...
                          level="M"
                          marginSize={1}
                          title="Flux pairing code QR"
                        />
                      </div>
                    ) : null}
                    <p className="mt-3 text-center text-xs text-muted-foreground">
                      {pairingStatus === "connected"
                        ? "Connected. This device has been paired."
                        : pairingStatus === "used"
                          ? "Code used. Waiting for the new client to finish connecting…"
                          : "Ready to scan or paste."}
                    </p>
                  </div>
                ) : null}
              </DialogPanel>
              <DialogFooter variant="bare">
                ...
                <Button disabled={isCreating} onClick={() => void createPairingCode()}>
                  {isCreating ? "Generating…" : pairingCode ? "Generate another" : "Generate code"}
                </Button>
              </DialogFooter>
            </DialogPopup>
          </Dialog>
...
function SavedBackendListRow({
  ...
  const metadataBits = [
    ...
    environment.relayManaged ? "Flux Connect" : null,
  ].filter((value): value is string => value !== null);
  ...
  // environment you connect to or remove here â€” its lifecycle is driven by the
      ...
      <div className={ITEM_ROW_INNER_CLASSNAME}>
        <div className="min-w-0 flex-1 space-y-1">
          ...
          {metadataBits.length > 0 ? (
            <p className="text-xs text-muted-foreground">{metadataBits.join(" Â· ")}</p>
          ) : null}
          <EnvironmentWebUrl url={environment.displayUrl} />
          {serverUpdateState.status !== "idle" ? (
            ...
            <Tooltip>
              ...
              <TooltipPopup side="top">
                {versionMismatch.serverVersion} <span aria-hidden="true">â†’</span>{" "}
          ...
          {isWslEnvironment ? (
            <Tooltip>
              ...
              <TooltipPopup side="top" className="max-w-80 whitespace-pre-wrap leading-tight">
                The WSL backend is managed by the WSL setting above â€” turn it on or off there.
              </TooltipPopup>
            </Tooltip>
          ) : (
            <>
...
function ConfiguredCloudLinkRow({ canManageRelay }: { readonly canManageRelay: boolean }) {
  ...
  const disabledReason = !isSignedIn
    ? "Sign in to Flux Connect to manage this environment."
    : !canManageRelay
      ? "Your session does not have permission to manage Flux Connect access."
      : null;
  ...
  const updateManagedTunnel = async (enabled: boolean) => {
    ...
    if (ok) {
      ...
      // rather than removing it â€” say so instead of claiming an unlink.
      toastManager.add({
        type: "success",
        title: enabled
          ? "Flux Connect linked"
          : publishAgentActivity
            ? "Flux Connect tunnel disabled"
            : "Flux Connect unlinked",
        description: enabled
          ? "This environment is available through Flux Connect."
          : publishAgentActivity
            ? "The managed tunnel was removed. Agent activity publishing stays on."
            : "This environment is no longer available through Flux Connect.",
      });
    }
    setIsUpdating(false);
  };
    ...
    <>
      {window.desktopBridge ? (
        <SettingsRow
          ...
          description={
            managedTunnelActive
              ? "This environment is available to your other devices through Flux Connect."
              : "Make this environment available to your other devices through Flux Connect."
          }
      ...
      <SettingsRow
        ...
        description="Send activity from this environment to your mobile clients for push notifications and Live Activities. Works without a Flux Connect tunnel."
        control={
          <CloudLinkSwitch
  ...
  return (
    <Empty className="min-h-52">
      ...
      <EmptyHeader>
        ...
        <EmptyDescription>
          {cloudEnabled
            ? "Click â€œAdd environmentâ€ to pair another environment, or connect one from Flux Connect."
            : "Click â€œAdd environmentâ€ to pair another environment."}
        </EmptyDescription>
      </EmptyHeader>
    </Empty>
  );
...
export function ConnectionsSettings() {
  ...
  // dropdown was confusing â€” they're the same decision.
  const handleSelectWslMode = useCallback(
    (value: string) => {
      ...
      if (value === BACKEND_VALUE_WSL_OFF) {
        ...
        // back to Windows and restart â€” always consequential).
      ...
      // Already enabled â€” treat as a distro switch. Skip the change if
  ...
  const handleToggleWslOnly = useCallback(
    (enabled: boolean) => {
      ...
      // Always prompt â€” even enabling is consequential here.
  ...
  const renderWslRow = () => {
        ...
        return (
          <SettingsRow
            ...
            control={
              ...
              >
                {isLoadingWslState ? "Retryingâ€¦" : "Retry"}
              </Button>
            }
          />
        );
    ...
    // there's nothing to recover â€” keep the section hidden as before.
    ...
    return (
      <>
        ...
        {desktopWslState.enabled ? (
          <SettingsRow
            title="WSL only"
            description="Stop the Windows backend and run only the WSL backend. Useful if you develop entirely inside WSL and don't want a second backend process. Flux restarts when you change this."
        ...
        <>
          <SettingsSection title="This environment">
            {primaryEnvironment?.displayUrl ? (
              <SettingsRow
                title="Web app"
                description="Open this environment in a browser or copy its local URL for another device."
                control={<EnvironmentWebUrl url={primaryEnvironment.displayUrl} />}
              />
            ) : null}
            <CloudflareTunnelRow />
                ...
                description={
                  primaryServerUpdateState.status !== "idle" ? (
                    ...
                    <Tooltip>
                      ...
                      />
                      <TooltipPopup side="top">
                        {primaryVersionMismatch.serverVersion} <span aria-hidden="true">â†’</span>{" "}
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingDesktopServerExposureMode === "network-accessible"
                    ? "Flux will restart to expose this environment over the network."
                    : "Flux will restart and limit this environment back to this machine."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
            ...
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  {pendingWslChange?.kind === "disable"
                    ? pendingWslChange.wasWslOnly
                      ? "Flux will restart on the Windows backend. Threads and projects opened against WSL stay safe inside the distro and become available again when you re-enable WSL."
                      : "The WSL backend will stop. Threads and projects opened against WSL stay safe inside the distro, but they'll be unavailable in Flux until you re-enable WSL."
                    : pendingWslChange?.kind === "distro"
                      ? "Flux will restart the WSL backend on the new distro. Sessions still running on the current distro will be interrupted."
                      : pendingWslChange?.kind === "enable"
                        ? "Run the WSL backend alongside the Windows one, or stop the Windows backend and use only WSL? You can change this later from Settings."
                        : pendingWslChange?.nextValue
                          ? "Flux will restart and start only the WSL backend. Your Windows-side projects won't be accessible until you turn this off again."
                          : "Flux will restart and bring the Windows backend back up alongside WSL."}
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <AlertDialogPopup>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Flux will restart the local backend without Tailscale Serve.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogClose
          ...
          >
            <DialogPopup className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Flux will restart the local backend with Tailscale Serve enabled and ask Tailscale

...
export function useSettingsRestore(onRestored?: () => void) {
  ...
  const isAgentDelegationDirty =
    settings.delegationEnabled !== DEFAULT_UNIFIED_SETTINGS.delegationEnabled ||
    settings.delegationInstructions !== DEFAULT_UNIFIED_SETTINGS.delegationInstructions ||
    !Equal.equals(settings.coordinatorModel, DEFAULT_UNIFIED_SETTINGS.coordinatorModel) ||
    !Equal.equals(
  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      ...(settings.enableAgentBrowserAccess !== DEFAULT_UNIFIED_SETTINGS.enableAgentBrowserAccess
        ...
        : []),
      ...(isAgentDelegationDirty ? ["Agent delegation"] : []),
    ],
    [
      isTextGenerationModelDirty,
      isBackgroundActivityDirty,
      isAgentDelegationDirty,
  ...
  const restoreDefaults = useCallback(async () => {
    ...
    updateSettings({
      ...
      delegationEnabled: DEFAULT_UNIFIED_SETTINGS.delegationEnabled,
      delegationInstructions: DEFAULT_UNIFIED_SETTINGS.delegationInstructions,
      coordinatorModel: DEFAULT_UNIFIED_SETTINGS.coordinatorModel,
      coordinatorBackupModels: DEFAULT_UNIFIED_SETTINGS.coordinatorBackupModels,
      agentProfiles: DEFAULT_UNIFIED_SETTINGS.agentProfiles,
    });
    onRestored?.();
  }, [
...
export function GeneralSettingsPanel() {
  ...
  const chooseAddProjectBaseDirectory = useCallback(async () => {
    const pickedPath = await window.desktopBridge?.pickFolder({
      ...(settings.addProjectBaseDirectory.trim().length > 0
        ? { initialPath: settings.addProjectBaseDirectory.trim() }
        : {}),
    });
    if (pickedPath) {
      updateSettings({ addProjectBaseDirectory: pickedPath });
    }
  }, [settings.addProjectBaseDirectory, updateSettings]);
            ...
            <div className="flex w-full gap-2 sm:w-72">
              ...
              {isElectron ? (
                <Button
                  ...
                  size="icon-sm"
                  variant="outline"
                  onClick={() => void chooseAddProjectBaseDirectory()}
                  aria-label="Choose project base folder"
                  title="Choose folder"
                >

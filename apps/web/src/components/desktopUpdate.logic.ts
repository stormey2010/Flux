...
const DESKTOP_RELEASE_TAG_URL = "https://github.com/stormey2010/Flux/releases/tag";

export function hasNewerMainCommit(state: DesktopUpdateState | null): boolean {
  return Boolean(
    state?.enabled &&
    state.currentCommitHash &&
    state.mainCommitHash &&
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  if (action === "download") {
    return "This Mac has Apple Silicon, but Flux is still running the Intel build under Rosetta. Download the available update to switch to the native Apple Silicon build.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but Flux is still running the Intel build under Rosetta. Restart to install the downloaded Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but Flux is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateInstallConfirmationMessage(
  ...
  return `Install update${version ? ` ${version}` : ""} and restart Flux?\n\nAny running tasks will be interrupted. Make sure you're ready before continuing.`;
}

...
function OpenCommandPaletteDialog(props: {
  ...
  useLayoutEffect(() => {
    ...
    const newProjectItem = {
      ...
      value: "action:new-thread-in:new-project",
      searchTerms: ["new project", "add project", "create", "folder"],
      title: "New project",
      icon: <FolderPlusIcon className={ITEM_ICON_CLASS} />,
      run: async () => {
        openAddProjectFlow();
    ...
    pushPaletteView({
      ...
      groups: [
        ...
        },
        {
          value: "new-project",
          label: "Create",
          items: enumerateCommandPaletteItems([newProjectItem]),
        },
      ],
    });
  }, [
  ...
  if (projects.length > 0) {
    actionItems.push({
      ...
      searchTerms: ["new thread", "project", "pick", "choose", "select", "chat", "create", "draft"],
  ...
  const openCreateFolderDialog = useCallback(() => {
    const parentPath = hasTrailingPathSeparator(query)
      ? (browseResult?.parentPath ?? browsePath.directoryPath)
      : browseDirectoryPath;
    if (parentPath.trim().length === 0) return;
    const suggestedName = hasTrailingPathSeparator(query)
      ? ""
      : browsePath.filterQuery.trim() || getBrowseLeafPathSegment(query).trim();
    setCreateFolderParentPath(parentPath);
    setCreateFolderName(suggestedName.replace(/[\\/]/gu, ""));
    setCreateFolderError(null);
    setIsCreateFolderDialogOpen(true);
  }, [browseDirectoryPath, browsePath, browseResult?.parentPath, query]);
  ...
  const submitCreateFolder = useCallback(async () => {
    ...
    if (name === "." || name === ".." || /[\\/]/u.test(name)) {
      setCreateFolderError("Use a folder name without path separators.");
      return;
    }
    setIsCreatingFolder(true);
    setCreateFolderError(null);
    try {
      await handleAddProject(appendBrowsePathSegment(createFolderParentPath, name));
      setIsCreateFolderDialogOpen(false);
    } finally {
      setIsCreatingFolder(false);
    }
  }, [createFolderName, createFolderParentPath, handleAddProject]);
      ...
      >
        <DialogPopup className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription>
              Name a new folder inside the directory you are browsing, then add it as your project.
            </DialogDescription>
          </DialogHeader>
          <DialogPanel scrollFade={false} className="space-y-3">
            ...
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-foreground">Folder name</span>
              <Input
                autoFocus
          ...
          <DialogFooter>
            ...
            <Button disabled={isCreatingFolder} onClick={() => void submitCreateFolder()}>
              {isCreatingFolder ? "Creating…" : "Create and add"}
            </Button>
          </DialogFooter>
        </DialogPopup>
      </Dialog>

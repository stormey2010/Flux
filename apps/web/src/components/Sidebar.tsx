...
import { ensureProjectAgentsMd } from "../ensureProjectAgentsMd";
import { PROJECT_AGENTS_MD_SETTINGS_HASH } from "../projectAgentsMd";
...
export default function Sidebar() {
  ...
  // Threads on non-primary environments (Flux Connect, hosted) resolve their
  ...
  const handleProjectContextMenu = useCallback(
    (event: ReactMouseEvent, projectGroup: SidebarProjectSnapshot) => {
      event.preventDefault();
      event.stopPropagation();
      void (async () => {
        const api = readLocalApi();
        if (!api) return;
        const members = projectGroup.memberProjects;
        const items =
          members.length === 1
            ? [
                { id: `create-prompt:${members[0]!.physicalProjectKey}`, label: "Create Prompt" },
                { id: "settings", label: "Project settings" },
              ]
            : [
                {
                  id: "create-prompt:submenu",
                  label: "Create Prompt",
                  children: members.map((member) => ({
                    id: `create-prompt:${member.physicalProjectKey}`,
        ...
        if (!clicked.value.startsWith("create-prompt:")) return;
        const physicalKey = clicked.value.slice("create-prompt:".length);
        const member = members.find((candidate) => candidate.physicalProjectKey === physicalKey);
        if (!member) return;
        const result = await ensureProjectAgentsMd({
        ...
        if (result.status === "failure") {
          ...
          toastManager.add(
            stackedThreadToast({
              type: "error",
              title: "Could not create project prompt",
              description: error instanceof Error ? error.message : "An error occurred.",
            }),
          );
          return;
        ...
        toastManager.add({
          type: "success",
          title: result.status === "created" ? "Created AGENTS.md" : "AGENTS.md already exists",
        ...
        void router.navigate({
          to: "/projects/$projectKey",
          params: { projectKey: projectGroup.projectKey },
          hash: PROJECT_AGENTS_MD_SETTINGS_HASH,
        });
      })();
    },
    [isMobile, router, setOpenMobile, writeProjectFile],
  );
  ...
  // New thread always opens the project picker. With no projects yet, open
  // create-project so the first chat starts from an explicit project.
  const handleNewThreadClick = useCallback(
    ...
    [isMobile, projectGroups.length, setOpenMobile],
  );

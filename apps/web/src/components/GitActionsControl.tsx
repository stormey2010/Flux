...
interface PublishRepositoryDialogProps {
  ...
  readonly initialProvider?: PublishProviderKind;
  readonly initialRepositoryName?: string;
  readonly initialStep?: number;
}

function defaultRepositoryName(cwd: string): string {
  const folderName = cwd.split(/[\\/]/).filter(Boolean).at(-1) ?? "project";
...
function PublishRepositoryDialog(props: PublishRepositoryDialogProps) {
  ...
  useEffect(() => {
    ...
    setSelectedPublishProvider(props.initialProvider ?? null);
    setPublishRepositoryOverride(null);
    setPublishWizardStep(props.initialStep ?? 0);
    setPublishAdvancedOpen(false);
    setPublishError(null);
    setPublishResult(null);
  }, [props.initialProvider, props.initialStep, props.open]);
  ...
  const publishRepositoryName = props.initialRepositoryName ?? null;
  const publishRepositoryPrefill = publishAccountByProvider[publishProvider]
    ? `${publishAccountByProvider[publishProvider]}/${publishRepositoryName ?? ""}`
...
export default function GitActionsControl({
  ...
  const [publishDialogMode, setPublishDialogMode] = useState<"manual" | "after-init">("manual");
  ...
  return (
    <>
            ...
            void (async () => {
              ...
              if (result._tag === "Success" || isAtomCommandInterrupted(result)) {
                if (result._tag === "Success") {
                  setPublishDialogMode("after-init");
                  setIsPublishDialogOpen(true);
                }
                return;
              }
      ...
      <PublishRepositoryDialog
        ...
        environmentId={activeEnvironmentId}
        gitCwd={gitCwd}
        {...(publishDialogMode === "after-init"
          ? {
              initialProvider: "github" as const,
              initialRepositoryName: defaultRepositoryName(gitCwd),
              initialStep: 1,
            }
          : {})}
      />

...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const configuredCoordinatorModel = settings.coordinatorModel ?? null;
  const resolvedCoordinatorModel = useMemo(() => {
    const candidates = [configuredCoordinatorModel, ...settings.coordinatorBackupModels].filter(
      (candidate): candidate is ModelSelection => candidate !== null,
    );
    for (const candidate of candidates) {
      const options = modelOptionsByInstance.get(candidate.instanceId) ?? [];
      if (
        options.some((option) => option.slug === candidate.model) &&
        !getModelDisabledReason(candidate.instanceId, candidate.model)
      ) {
        return candidate;
      }
    }
    ...
    settings.coordinatorBackupModels,
  ]);
  ...
  const [isDefaultAgentSelected, setIsDefaultAgentSelected] = useState(false);
  ...
  const providerTraitsMenuContent = isDefaultAgentSelected
    ? null
    : renderProviderTraitsMenuContent({
        provider: selectedProvider,
  ...
  const providerTraitsPicker = isDefaultAgentSelected
    ? null
    : renderProviderTraitsPicker({
        provider: selectedProvider,
        instanceId: selectedInstanceId,
  ...
  useEffect(() => {
    ...
    setIsDefaultAgentSelected(false);
  ...
  // Render
  // ------------------------------------------------------------------
  const handlePickerModelChange = useCallback(
  ...
  const handleDefaultAgentSelect = useCallback(() => {
    setSelectedAgentId(null);
    setIsDefaultAgentSelected(true);
    if (resolvedCoordinatorModel) {
      onProviderModelSelect(resolvedCoordinatorModel.instanceId, resolvedCoordinatorModel.model);
    }
  }, [onProviderModelSelect, resolvedCoordinatorModel]);
  const handleAgentSelect = useCallback(
    (agentId: string) => {
      const profile = settings.agentProfiles.find((candidate) => candidate.id === agentId);
      if (!profile || !profile.enabled) return;
      setSelectedAgentId(profile.id);
      setIsDefaultAgentSelected(true);
      const selection = profile.primaryModel ?? resolvedCoordinatorModel;
      if (selection) {
        onProviderModelSelect(selection.instanceId, selection.model);
      }
    },
    [onProviderModelSelect, resolvedCoordinatorModel, settings.agentProfiles],
  );
                ...
                <div className="-m-1 -ms-3.5 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto p-1 ps-3.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  {noProviderAvailable ? (
                    ...
                    <ProviderModelPicker
                      ...
                      selectedAgentName={
                        selectedAgentId
                          ? (settings.agentProfiles.find(
                              (profile) => profile.id === selectedAgentId,
                            )?.name ?? null)
                          : null
                      }
                      ...
                      onInstanceModelChange={handlePickerModelChange}
                      onDefaultAgentSelect={handleDefaultAgentSelect}

...
function isEmptyCompletedReasoningEntry(entry: TimelineEntry | undefined): boolean {
  return (
    entry?.kind === "message" &&
...
export function deriveMessagesTimelineRows(input: {
  ...
  const hasTrailingEmptyCompletedReasoning = isEmptyCompletedReasoningEntry(
    input.timelineEntries.at(-1)!,
  );
  ...
  for (let index = 0; index < timelineEntries.length; index += 1) {
    ...
    if (
      input.isWorking &&
      index === activeTurnHeaderIndex &&
      (activeWorkRow === null || activeTurnHasVisibleContent || !hasTrailingEmptyCompletedReasoning)
    ) {
      appendWorkingRow();
    }

...
import { BotIcon, ChevronRightIcon, SearchIcon } from "lucide-react";
...
const DEFAULT_AGENT_ITEM_KEY = "__t3_default_agent__";
const agentItemKey = (id: string) => `__agent__${id}`;
...
export const ModelPickerContent = memo(function ModelPickerContent(props: {
  ...
  coordinatorModel?: { readonly instanceId: ProviderInstanceId; readonly model: string } | null;
  onDefaultAgentSelect?: () => void;
  ...
  const agentProfiles = primarySettings.agentProfiles;
  const [selectedInstanceId, setSelectedInstanceId] = useState<
    ProviderInstanceId | "favorites" | "agents"
  >(() => {
  ...
  const handleSelectInstance = useCallback(
    (instanceId: ProviderInstanceId | "favorites" | "agents") => {
  ...
  const showSidebar =
    !isSearching && (sidebarInstanceEntries.length > 0 || agentProfiles.length > 0);
  ...
  const legacySection = useMemo(() => {
    if (isSearching || selectedInstanceId === "favorites" || selectedInstanceId === "agents") {
      return null;
    }
  ...
  const allItemKeys = useMemo(
    (): string[] => [
      DEFAULT_AGENT_ITEM_KEY,
      ...agentProfiles.map((profile) => agentItemKey(profile.id)),
  ...
  const filteredItemKeys = useMemo((): string[] => {
    ...
    if (selectedInstanceId === "agents") {
      ...
      const profiles = query
        ...
        : agentProfiles;
      return [DEFAULT_AGENT_ITEM_KEY, ...profiles.map((profile) => agentItemKey(profile.id))];
    }
    if (!legacySection) {
      return selectedInstanceId === "favorites" && !isSearching && props.lockedProvider === null
        ? [DEFAULT_AGENT_ITEM_KEY, ...modelKeys]
        : modelKeys;
    }
      ...
      >
        ...
        <Combobox
          ...
          onValueChange={(modelKey) => {
            ...
            if (modelKey === DEFAULT_AGENT_ITEM_KEY) {
              onDefaultAgentSelect?.();
              return;
            }
            if (modelKey.startsWith("__agent__")) {
              onAgentSelect?.(modelKey.slice("__agent__".length));
              return;
            }
                ...
                <ComboboxInput
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (e.key === "Enter" && highlightedModelKeyRef.current) {
                      ...
                      if (highlightedModelKeyRef.current === DEFAULT_AGENT_ITEM_KEY) {
                        onDefaultAgentSelect?.();
                        return;
                      }
                      if (highlightedModelKeyRef.current.startsWith("__agent__")) {
                        onAgentSelect?.(highlightedModelKeyRef.current.slice("__agent__".length));
                        return;
                      }
              ...
              <ComboboxListVirtualized className="size-full min-w-0 p-0 not-empty:p-0">
                <LegendList<string>
                  ...
                  renderItem={({ item: modelKey, index }) => {
                    if (modelKey === DEFAULT_AGENT_ITEM_KEY) {
                      const coordinatorLabel = props.coordinatorModel
                        ? `${props.coordinatorModel.instanceId} · ${props.coordinatorModel.model}`
                        : "Uses the chat model and your agent workflow settings";
                      return (
                        <ComboboxItem
                          hideIndicator
                          index={index}
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1 text-left">
                            <div className="text-xs font-medium leading-snug">Default agent</div>
                            <div className="mt-1 truncate text-xs font-normal leading-snug text-muted-foreground/70">
                              {coordinatorLabel}
                            </div>
                          </div>
                          {props.isDefaultAgentSelected && !props.selectedAgentId ? (
                    ...
                    if (modelKey.startsWith("__agent__")) {
                      const profile = agentProfiles.find(
                        (candidate) => agentItemKey(candidate.id) === modelKey,
                      );
                      ...
                      return (
                        <ComboboxItem
                          ...
                          className={cn(
                            "group w-full cursor-pointer rounded-md px-2 py-2",
                            props.selectedAgentId === profile.id && "bg-accent/60",
                          )}
                          contentClassName="flex w-full items-center gap-3"
                        >
                          <div className="flex size-7 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary">
                          ...
                          <div className="min-w-0 flex-1 text-left">
                            <div className="text-xs font-medium leading-snug">{profile.name}</div>
                            <div className="mt-1 truncate text-xs font-normal leading-snug text-muted-foreground/70">
                              {profile.description || "Custom Flux agent"}
                            </div>
                          </div>
                          {props.selectedAgentId === profile.id ? (
                            <span className="text-[11px] text-primary">Active</span>
                          ) : null}
                        </ComboboxItem>
                      );

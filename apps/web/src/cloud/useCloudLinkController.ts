...
/**
 * Drives the primary environment's Flux Connect link. Flux Connect (managed
...
export function useCloudLinkController() {
  ...
  const reportUpdateFailure = (cause: unknown) => {
    const message =
      cause instanceof Error ? cause.message : "Could not update Flux Connect access.";
    const traceId = findErrorTraceId(cause);
    console.error("[flux-connect] Could not update Flux Connect", { message, traceId, cause });
    setOperationError(traceId ? `${message} Trace ID: ${traceId}` : message);
    toastManager.add({
      type: "error",
      title: "Could not update Flux Connect",
      description: message,
      data: traceId
        ? {
  ...
  const reconcileCloudState = async (desired: CloudLinkDesiredState): Promise<boolean> => {
    ...
    if (!wantsLink) {
      ...
      // leave the user unable to turn Flux Connect off.
      ...
      if (!clerkToken) {
        reportUpdateFailure(new Error("Sign in to Flux Connect before enabling this."));
        return false;
      }

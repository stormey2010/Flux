  ...
  return Effect.gen(function* () {
    ...
    if (status.status === "unsupported") {
      return yield* new CloudEnvironmentLinkError({
        message: `Flux cannot install the relay client automatically on ${status.platform}-${status.arch}.`,
      });
    }
      ...
      return yield* new CloudEnvironmentLinkError({
        message:
          installedStatus.status === "unsupported"
            ? `Flux cannot install the relay client automatically on ${installedStatus.platform}-${installedStatus.arch}.`
...
// independently of Flux Connect while clients reach the environment out of band.

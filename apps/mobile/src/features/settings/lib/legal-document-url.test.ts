...
describe("isLegalDocumentUrl", () => {
  it.each([
    "https://flux.codes/legal",
    "https://flux.codes/legal/",
    "https://flux.codes/privacy-policy?source=app",
    "https://flux.codes/terms-of-service#updates",
    "https://flux.codes/security-policy",
  ])("allows a configured legal document: %s", (url) => {
    expect(isLegalDocumentUrl(url)).toBe(true);
  });

  it.each([
    "https://flux.codes/download",

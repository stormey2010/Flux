...
function ConfiguredSettingsRouteScreen() {
    ...
    if (result.value.type === "granted") {
      ...
      if (getAgentAwarenessRegistrationStatus() === "registered") {
        ...
        Alert.alert(
          ...
          "Notification access was granted, but this device could not be registered with Flux Connect. Notifications will start once registration succeeds.",
        );
      }
      return;
    }
  ...
  const promptSignIn = useCallback(() => {
    Alert.alert(
      "Sign in to Flux Connect",
      "Live Activity updates require Flux Connect so relay can deliver updates to this device.",
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Continue",
  ...
  const linkEnvironments = useCallback(async () => {
    ...
    if (getAgentAwarenessRegistrationStatus() === "registered") {
      ...
      Alert.alert(
        ...
        "This device could not be registered with Flux Connect, so Live Activities won't appear yet. They'll start once registration succeeds.",
      );
    }
  }, [
    connections,
  ...
  const handleDeviceNotificationsChange = useCallback(
    (enabled: boolean) => {
      ...
      Alert.alert(
        ...
        "Notification permission is controlled by iOS. Open Settings to disable notifications for Code.",
        [
          { text: "Cancel", style: "cancel" },
          { text: "Open Settings", onPress: () => void Linking.openSettings() },
        ],
      ...
      >
        <View className="gap-3">
          ...
          <Text className="px-2 text-sm text-foreground-muted">
            Code works locally without signing in. Cloud features are optional.
          </Text>
        </View>

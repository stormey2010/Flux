...
interface CloudEnvironmentRowsProps {
  ...
  /**
   * Hide the "Flux Connect" section title + refresh button for hosts that
...
/**
 * "Flux Connect" section: every environment published to the signed-in account,
 * with connect switches, availability status, refresh, and loading/error
 * states. Shared between the Settings environments screen and the Flux Connect
 * onboarding sheet.
 *
    ...
    <View collapsable={false} className={cn("gap-3", showHeader && "mt-5")}>
      {showHeader ? (
        <View className="flex-row items-center justify-between px-1">
          <Text className="text-sm font-t3-bold uppercase text-foreground-muted">Flux Connect</Text>
          {discoveryAvailable ? (
            <Pressable
      ...
      !controller.relayDiscovery.isRefreshing ? (
        <View collapsable={false} className="gap-3 rounded-[24px] bg-card p-5">
          <Text className="text-base font-t3-bold text-foreground">
            Could not load Flux Connect environments
          </Text>
